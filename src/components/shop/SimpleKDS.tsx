...
export default function SimpleKDS() {
  ...
  useEffect(() => {
    if (shopId) {
      import('@/lib/supabase/queries/shop-dashboard').then(m => {
        m.getShopOrderHistory(shopId, 20).then(data => {
          setHistory(data)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 opacity-75 hover:opacity-100 transition-opacity">
            {history.map(order => (
              <button
                key={order.id}
                onClick={() => setFocusedOrderId(order.id)}
                className={`bg-white p-5 rounded-3xl shadow-sm border ${order.status === 'cancelled' ? 'border-red-100' : 'border-gray-200'} flex flex-col gap-3 text-left w-full`}
              >
